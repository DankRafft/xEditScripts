...
begin
  ...
  i := MessageDlg('Prepend [YES] or append [NO] to Name?', mtConfirmation, [mbYes, mbNo, mbCancel], 0);
...
var
  elName: IInterface;
begin
  elName := ElementBySignature(e, 'FULL');
  
  if Assigned(elName) then
    if DoPrepend then begin
      if not StartsText(s, GetEditValue(elName)) then
        SetEditValue(elName, s + GetEditValue(elName));
    end
    else begin
      if not EndsText(s, GetEditValue(elName)) then
        SetEditValue(elName, GetEditValue(elName) + s);
    end;
end;
